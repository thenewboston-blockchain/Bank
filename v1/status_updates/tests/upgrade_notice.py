...
def set_primary_vaidator(client, self_configuration, signing_key, status):
...
def test_upgrade_notice_200(client, self_configuration, validator, signing_key, celery_worker):
    ...
    assert set_primary_vaidator(client, self_configuration, signing_key, HTTP_200_OK) == {}
...
def test_upgrade_notice_200_same_validator(client, self_configuration, validator, signing_key, celery_worker):
    ...
    set_primary_vaidator(client, self_configuration, signing_key, HTTP_200_OK)
    set_primary_vaidator(client, self_configuration, signing_key, HTTP_200_OK)
...
def test_upgrade_notice_400_low_trust(client, self_configuration, validator, signing_key, celery_worker):
    ...
    response = set_primary_vaidator(client, self_configuration, signing_key, HTTP_400_BAD_REQUEST)
